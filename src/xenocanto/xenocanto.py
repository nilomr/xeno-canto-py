...
def tqdmm(iterable: Iterable[Any], desc: str | None = None, **kwargs) -> tqdm:
    ...
    Returns:
        tqdm: progress bar.
    """
    return tqdm(iterable, desc=desc, leave=True, position=0, file=sys.stdout, **kwargs)


def metadata(filt: List[str], out_directory: str | os.PathLike = "dataset") -> Path:
    """
...
def _download_files(
    meta: List[Dict[str, Any]], redown: Set, path: Path
    ...
    for data in tqdmm(meta, desc="Downloading files", total=len(meta)):
        ...
        # FIXME@nilomr: this is a slightly strange to deal with bad downloads.
...
def download(filt: List[str], out_directory: str | os.PathLike = "dataset") -> None:
    """
    ...
    print(f"Found {len(recordings)} files matching your query, " "download will start:")
    ...
    if len(existing_files):
        num_form = ["is", "was"] if len(existing_files) == 1 else ["are", "were"]
        print(
...
def purge(
    num: int, out_directory: str | os.PathLike = "dataset", verbose: bool = True
) -> None:
    """
...
def delete(filt: List[str], out_directory: str | os.PathLike = "dataset") -> None:
    """
...
def gen_meta(out_directory: str | os.PathLike = "dataset"):
    ...
    meta_files = [
        filename
        for filename in _listdir_nohidden(metadata_dir)
    ...
    print(
        "You can find the library metadata file at " f"{metadata_dir / 'library.json'}"
    )
...
def main():
    ...
    if act == "-m":
        ...
        print(
            "The command entered was not found, please consult the README "
